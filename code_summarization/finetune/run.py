...
from transformers import AdamW, get_linear_schedule_with_warmup, RobertaTokenizer, T5ForConditionalGeneration
from codecarbon import EmissionsTracker


MODEL_CLASSES = {"codet5": (RobertaTokenizer, T5ForConditionalGeneration)}
...
def main():
    ...
    parser.add_argument(
        "--load_model_path", default=None, type=str, help="Path to trained model: Should contain the .bin files"
    )
    ## Other parameters
    parser.add_argument(
        "--train_filename",
    ...
    if args.do_train:
        ...
        train_dataloader = DataLoader(
            train_data, sampler=train_sampler, batch_size=args.train_batch_size // args.gradient_accumulation_steps
        )
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        preds = model.module.generate(
                            input_ids=source_ids, attention_mask=source_mask, max_length=args.max_target_length
                        )
                        for pred in preds:
    ...
    if args.do_test:
	    ...
	    tracker.start()
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                ...
                source_ids, source_mask = batch
                with torch.no_grad():
                    # Use generate method for summarization
	    ...
	    print(f"\nCarbon emissions for inference: {emissions:.6f} kg CO2")
            ...
            # Save and prepare BLEU evaluation
            output_file = os.path.join(args.output_dir, f"test_{idx}.output")
            gold_file = os.path.join(args.output_dir, f"test_{idx}.gold")
            with open(output_file, "w") as f, open(gold_file, "w") as f1:
                for ref_text, example in zip(generated_texts, eval_examples):
                    predictions.append(str(example.idx) + "\t" + ref_text)
                    f.write(f"{example.idx}\t{ref_text}\n")
                    f1.write(f"{example.idx}\t{example.target}\n")

            # Calculate BLEU score
            (goldMap, predictionMap) = bleu.computeMaps(predictions, gold_file)
            test_bleu = round(bleu.bleuFromMaps(goldMap, predictionMap)[0], 2)
            logger.info(f"  bleu-4 = {test_bleu}")
            logger.info("  " + "*" * 20)
